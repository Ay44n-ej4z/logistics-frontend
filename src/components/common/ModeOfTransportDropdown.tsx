...
export default function ModeOfTransportDropdown({ value, onChange, error, disabled, required = true }: Props) {
  const selected = transportModes.find((mode) => mode.id === value) || transportModes[0];

  return (
    <div className="w-full">
      <Listbox value={value} onChange={(val) => onChange(val as TransportMode)} disabled={disabled}>
